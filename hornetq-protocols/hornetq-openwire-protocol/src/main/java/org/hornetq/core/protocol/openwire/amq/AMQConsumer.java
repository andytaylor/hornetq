...
public class AMQConsumer implements BrowserListener
{
   ...
   public void init()
   {
      ServerSession coreSession = session.getCoreSession();
      ...
      try
      {
         ...
         if (this.actualDest.isTopic())
         {
            ...
            ((ServerSessionImpl)coreSession).createConsumer(nativeId, subQueueName, null, info.isBrowser(), false, Integer.MAX_VALUE, this);
         }
         else
         {
            SimpleString queueName = new SimpleString("jms.queue." + this.actualDest.getPhysicalName());
            ((ServerSessionImpl)coreSession).createConsumer(nativeId, queueName, selector, info.isBrowser(), false, Integer.MAX_VALUE, this);
         }
      }
      catch (Exception e)
      {
   ...
   public void acquireCredit(int n) throws Exception
   {
      ...
      {
         AtomicInteger credits = ((ServerSessionImpl)session.getCoreSession()).getConsumerCredits(nativeId);
         credits.set(0);
         ((ServerSessionImpl)session.getCoreSession()).receiveConsumerCredits(nativeId, Integer.MAX_VALUE);
      }
   }
   ...
   public void checkCreditOnDelivery() throws Exception
   {
      ...
      if (currentSize.get() == prefetchSize)
      {
         ...
         ((ServerSessionImpl)session.getCoreSession()).receiveConsumerCredits(nativeId, 0);
      }
   }
